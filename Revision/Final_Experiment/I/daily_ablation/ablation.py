...
def denormalize_close(scaled_val, scaler, num_features):
    ...
    dummy[0, 3] = scaled_val  # Close在索引3
...
# =========================================================
def run_experiment(exp_name, use_nhits, use_auto, loaders, scaler, num_features, test_target_dates=None,
                   output_file=None, **model_kwargs):
...
if __name__ == "__main__":
    ...
    # Extract test_target_dates when preprocessing
    ...
    def random_search_daily(loaders, scaler, num_features, num_trials=6):
        ...
        for i in range(num_trials):
            ...
            # Don't save individual CSV files for random search trials
    ...
    # Experiment 1: Base
    res_base = run_experiment("Base (DLinear Only)", False, False, loaders, scaler, num_features,
    ...
    # Experiment 2: Base + Autoformer
    res_auto = run_experiment("Base + Autoformer", False, True, loaders, scaler, num_features,
                              test_target_dates, output_file=os.path.join(OUTPUT_DIR, "pred_daily_base_as1.csv"),
                              **best_params)
    results.append(res_auto)
    pd.DataFrame(results).to_csv(OUTPUT_METRICS_FILE, index=False)

    # Experiment 3: Base + N-HiTS
    res_nhits = run_experiment("Base + N-HiTS (Opt)", True, False, loaders, scaler, num_features,
                               test_target_dates, output_file=os.path.join(OUTPUT_DIR, "pred_daily_base_as2.csv"),
                               **best_params)
    results.append(res_nhits)
    pd.DataFrame(results).to_csv(OUTPUT_METRICS_FILE, index=False)

    # Experiment 4: Full Hybrid Model
    res_full = run_experiment("Full Hybrid Model (Opt)", True, True, loaders, scaler, num_features,
    ...
    print(f"Results successfully saved to {OUTPUT_METRICS_FILE}")
