...
OUTPUT_FILE_PRED = "../strategy/pred_daily_advanced.csv"
...
# ==============================================================
# 新增参数：test_target_dates
def run_experiment(exp_name, use_nhits, use_auto, loaders, scaler, num_features, test_target_dates, **model_kwargs):
    ...
    if exp_name == "DENet":
        ...
        os.makedirs(os.path.dirname(OUTPUT_FILE_PRED), exist_ok=True)
        pred_df.to_csv(OUTPUT_FILE_PRED, index=False)
        print(f"DENet predictions saved to {OUTPUT_FILE_PRED}")
    # ========================================================================
...
if __name__ == "__main__":
    ...
    def random_search_daily(loaders, scaler, num_features, num_trials=6):
        ...
        for i in range(num_trials):
            ...
            res = run_experiment(f"Trial_{i}", True, True, loaders, scaler, num_features, test_target_dates,**config)
    ...
    # 所有实验调用传入test_target_dates
    res_base = run_experiment("Base", False, False, loaders, scaler, num_features, test_target_dates,**best_params)
    results.append(res_base)

    res_auto = run_experiment("Base + AS1", False, True, loaders, scaler, num_features, test_target_dates,**best_params)
    results.append(res_auto)

    res_nhits = run_experiment("Base + AS2", True, False, loaders, scaler, num_features, test_target_dates,**best_params)
    results.append(res_nhits)

    # 完整模型DENet，会自动保存预测结果
    res_full = run_experiment("DENet", True, True, loaders, scaler, num_features, test_target_dates, **best_params)
...
0        Base 29.5476 38.5943 0.9752       53.9503
1  Base + AS1 28.7315 38.0337 0.9759       50.7901
2  Base + AS2 28.6136 37.9651 0.9760       53.0474
3       DENet 28.5451 37.9334 0.9760       54.6275
============================================================
"""