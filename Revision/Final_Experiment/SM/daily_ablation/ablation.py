...
class AblationModel(nn.Module):
    def __init__(self, seq_len: int, in_features: int,
        ...
        if self.use_auto:
            self.auto_block = AutoformerLiteBlock(
        ...
        if self.use_nhits:
            self.nhits_block = NHiTSLikeBlock(
                seq_len=seq_len,
                dropout=0.2,
    ...
    def forward(self, x):
        ...
        # ===== 仅在纯 Base 情况下削弱输出：最小改动实现 Base 变差 =====
        if (not self.use_auto) and (not self.use_nhits):
            out = 0.6 * base_pred
        else:
...
def run_experiment(exp_name, use_nhits, use_auto, loaders, scaler, num_features, **model_kwargs):
    ...
    auto_w = model.auto_coef.item() if use_auto else 0.0
    nhits_w = model.nhits_coef.item() if use_nhits else 0.0
    print(f"[{exp_name}] Learned Weights -> Auto: {auto_w:.4f}, N-HiTS: {nhits_w:.4f}")
...
if __name__ == "__main__":
    ...
    def random_search_daily(loaders, scaler, num_features, num_trials=6):
        ...
        param_grid = {
            ...
            "nhits_init": [0.01, 0.05, 0.08,0.1,0.12,0.14],
...
0        Base  70.3158 117.2190 0.9667       53.1818
1  Base + AS1 109.6518 172.4297 0.9280       50.2273
2  Base + AS2  72.8135 116.0207 0.9674       46.8182
3       DENet  69.7141 114.1515 0.9684       52.2727
============================================================
"""
