...
def parse_metrics(output):
    ...
    calmar_match = re.search(r"Calmar ratio:\s*(-?\d+\.?\d*)", output)
    

    total_trades_match = re.search(r"Total number of trades \(closed\):\s*(\d+)", output)
    won_trades_match = re.search(r"Number of winning trades:\s*(\d+)", output)
    lost_trades_match = re.search(r"Number of losing trades:\s*(\d+)", output)
    ...
    calmar_ratio = float(calmar_match.group(1)) if calmar_match else 0.0
    

    total_trades = int(total_trades_match.group(1)) if total_trades_match else 0
    won_trades = int(won_trades_match.group(1)) if won_trades_match else 0
    lost_trades = int(lost_trades_match.group(1)) if lost_trades_match else 0
...
def run_backtest(
    ...
    if not use_trailing_stop:
        cmd.append("--disable-trailing-stop")
    else:
        cmd.extend(["--atr-period", str(atr_period)])
        cmd.extend(["--atr-multiplier", str(atr_multiplier)])
...
def optimize_on_prediction_period(
    ...
    print(f"Optimization Period: {train_start} to {train_end}")
    print(f"=======================================================")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_params):
            ...
            won_t = metrics["won_trades"]
            lost_t = metrics["lost_trades"]
            ...
            if annual_ret > best_annual_return:
                ...
                print(
                    f"*** New Best for Advanced_DENet: Annualized return rate: {annual_ret:.2f}% | "
                    f"Total return rate: {total_ret:.2f}% | "
                    f"Sharpe: {sharpe:.2f} | "
                    f"Calmar ratio: {sharpe:.2f} | Win: {win:.2f}% | DD: {dd:.2f}% | "
                    f"Trades(Total/Win/Loss): {tot_t}/{won_t}/{lost_t} "
    ...
    if best_params:
        ...
        print(f"Winning Trades: {best_metrics['won_trades']}")
        print(f"Losing Trades: {best_metrics['lost_trades']}")
...
def compare_strategies_on_prediction_period(
    ...
    print("\nRunning Original R-Breaker ")
    original_metrics = run_backtest(
        f1, f2, f3, eval_start, eval_end, minute_data_path, prev_daily_path,
        daily_pred=None, minute_pred=None, enable_dynamic_sizing=False,
        enable_prediction_filter=False, slippage_perc=slippage_perc,
        use_trailing_stop=use_trailing_stop, atr_period=atr_period, atr_multiplier=atr_multiplier
    )

    print("\nRunning Prediction-Enhanced R-Breaker...")
    enhanced_metrics = run_backtest(
        f1, f2, f3, eval_start, eval_end, minute_data_path, prev_daily_path,
        daily_pred=daily_pred, minute_pred=minute_pred, enable_dynamic_sizing=True,
        enable_prediction_filter=False, slippage_perc=slippage_perc,
        use_trailing_stop=use_trailing_stop, atr_period=atr_period, atr_multiplier=atr_multiplier
    )
    ...
    print(f"Calmar ratio: {original_metrics['calmar_ratio']:.2f}")
    ...
    print(f"Winning trades: {original_metrics['won_trades']}")
    print(f"Losing trades: {original_metrics['lost_trades']}")
    ...
    print(f"Calmar ratio: {enhanced_metrics['calmar_ratio']:.2f}")
    ...
    print(f"Winning trades: {enhanced_metrics['won_trades']}")
    print(f"Losing trades: {enhanced_metrics['lost_trades']}")
    ...
    comparison_df = pd.DataFrame([
        {
            ...
            "won_trades": original_metrics["won_trades"],
            "lost_trades": original_metrics["lost_trades"]
        },
        {
            ...
            "won_trades": enhanced_metrics["won_trades"],
            "lost_trades": enhanced_metrics["lost_trades"]
        }
    ])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--minute-data-path", default="SM_5.csv")
    ...
    parser.add_argument("--train-start", default="2023-5-15")
    ...
    parser.add_argument("--disable-trailing-stop", action="store_true", help="Disable ATR based trailing stop")
    parser.add_argument("--atr-period", type=int, default=14, help="ATR period for trailing stop")
    parser.add_argument("--atr-multiplier", type=float, default=2.0, help="ATR multiplier for trailing stop")

    args = parser.parse_args()
    model_name = "Advanced_DENet"
    use_ts = not args.disable_trailing_stop
...
Fixed Parameters from Training: f1=0.80, f2=0.10, f3=0.40
=======================================================
...
[Original R-Breaker]
Annualized return rate: 137.53%
Total return rate: 72.61%
Sharpe ratio: 2.15
Calmar ratio: 6.61
Win rate: 39.29%
...
[Prediction-Enhanced R-Breaker]
Annualized return rate: 143.53%
Total return rate: 75.35%
Sharpe ratio: 2.19
Calmar ratio: 8.51
Win rate: 39.29%
