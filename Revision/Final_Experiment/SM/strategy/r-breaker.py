...
class ImprovedRBreakerStrategy(bt.Strategy):
    """Improved R-Breaker Strategy with Trailing Stop"""
    params = (
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            if order == self.close_order:
                ...
                self.log(
                    f'Trailing stop executed: price={order.executed.price:.2f}, quantity={order.executed.size:.0f}')
                self.stop_order = None
        else:
...
def run_backtest(f1, f2, f3, data_path, prev_daily_path, minute_pred_path=None, daily_pred_path=None,
                 ...
                 prediction_exit_gap=0.0, use_trailing_stop=True, atr_period=14, atr_multiplier=2.0,
                 initial_capital=100000, commission=0.0001, slippage_perc=0.0, mult=10,
    ...
    if use_trailing_stop:
        print(f'Trailing Stop: ENABLED (ATR Period: {atr_period}, Multiplier: {atr_multiplier})')
    ...
    print(f'Total number of trades (closed): {total_trades}')
    print(f'Number of winning trades: {won_trades}')
    print(f'Number of losing trades: {lost_trades}')
    ...
    calmar_ratio = annual_return / drawdown if drawdown > 0 else 0.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--atr-period', type=int, default=14, help='ATR period for trailing stop (default: 14)')
    ...
    run_backtest(
        ...
        use_trailing_stop=not args.disable_trailing_stop,
