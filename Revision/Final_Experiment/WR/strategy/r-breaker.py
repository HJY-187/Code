...
def run_backtest(f1, f2, f3, data_path, prev_daily_path, minute_pred_path=None, daily_pred_path=None,
    ...
    daily_returns = pd.Series(returns_dict).dropna().astype(float)
    ...
    if len(daily_returns) > 1:
        ...
        if std_dev > 0:
            total_compound_return = final_cash / initial_cash - 1
            num_days = len(daily_returns)
            annualized_return_decimal = (1 + total_compound_return) ** (trading_days / num_days) - 1
            annualized_volatility = std_dev * np.sqrt(trading_days)
            sharpe_ratio = (annualized_return_decimal - risk_free_annual) / annualized_volatility
        else:
            sharpe_ratio = 0.0
    else:
        sharpe_ratio = 0.0
