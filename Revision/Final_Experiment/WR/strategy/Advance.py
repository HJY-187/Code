...
def parse_metrics(output):
    """
    根据确定的 Backtrader 纯英文输出格式，进行精准正则匹配提取
    """
    ...
    # 兼容解析交易次数（忽略大小写，并兼容有无 (closed) 以及 losing/lost 的写法）
    total_trades_match = re.search(r"Total.*trades.*\:\s*(\d+)", output, re.IGNORECASE)
    won_trades_match = re.search(r"Number of win.*trades:\s*(\d+)", output, re.IGNORECASE)
    lost_trades_match = re.search(r"Number of los.*trades:\s*(\d+)", output, re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--minute-data-path", default="WR_5.csv")
...
"""
Fixed Parameters from Training: f1=0.70, f2=0.20, f3=0.10
[Original R-Breaker]
Annualized return rate: -18.66%
Total return rate: -12.21%
Sharpe ratio: -0.17
Calmar ratio: -0.27
Win rate: 52.50%
...
[Prediction-Enhanced R-Breaker]
Annualized return rate: 17.41%
Total return rate: 10.66%
Sharpe ratio: 0.11
Calmar ratio: 0.23
Win rate: 52.50%
Maximum drawdown: 75.34%
Total trades: 40
Winning trades: 21
Losing trades: 0
"""