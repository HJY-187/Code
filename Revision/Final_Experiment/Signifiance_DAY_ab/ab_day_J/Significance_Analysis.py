...
CONFIG = {
    "file_paths": {
        "true": os.path.join(base_dir, "J_true.csv"),
        "DENet": os.path.join(base_dir, "pred_daily_full.csv"),
        "baseline": os.path.join(base_dir, "pred_daily_base.csv"),
        "AS1": os.path.join(base_dir, "pred_daily_base_as1.csv"),
        "AS2": os.path.join(base_dir, "pred_daily_base_as2.csv"),
        
    },
    "models": ["DENet", "baseline", "AS1", "AS2"],
    "base_model": "DENet",
...
"""
PS D:\桌面\trea_Run> & D:/python/python.exe d:/桌面/trea_Run/DAY_ab_Sig/ab_day_J/Significance_Analysis.py
Reading true values...
Reading model: DENet...
...
================================================================================
Automatic prediction alignment check (based on Base vs True MSE)
  shift=+0 day(s) | MSE=2073.259563 | n=443
  shift=+1 day(s) | MSE=4133.620212 | n=348
Suggested alignment: shift=+0 (lowest MSE)
Hint: shift=+0 improves MSE by about 49.84% over second best
   This usually indicates potential misalignment between saved date and target prediction date.        
   Please manually verify using the sample below.

Sample verification (first few aligned days):
  datetime  true_close  DENet_pred  baseline_pred
2023-03-08      2834.5 2907.764954    2898.264446
2023-03-09      2933.0 2821.654370    2856.292489
2023-03-10      2927.5 2915.874121    2864.110347
   Note: Check whether pred appears to predict the corresponding date's true value.
================================================================================


================================================================================
Starting pairwise significance test (HAC mean test / DM-GW style)
Test direction: diff = loss_comp - loss_base
   H1: E[diff] > 0  => comp worse / base better (t>0 supports base better)
Current alignment used: shift=+0 day(s)
   Practical tip: GW_stat > 1.645 usually indicates one-sided 0.05 significance (large sample approx)  
================================================================================
...
Test Result Summary (one-sided H1: base better, FDR-BH corrected)
   model  n_samples  alignment_shift_days   base_loss   comp_loss  diff_mean GW_stat p_one(H1:base_better)        p_fdr  is_significant  lags direction_by_t
     AS1        443                     0 2073.259563 2246.457638 173.198075  2.6426          4.113822e-03 4.113822e-03            True     5    base_better
     AS2        443                     0 2073.259563 2640.357516 567.097953  5.1570          1.254426e-07 1.881640e-07            True     5    base_better
baseline        443                     0 2073.259563 2831.954737 758.695174  5.3750          3.829404e-08 1.148821e-07            True     5    base_better
