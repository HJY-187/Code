...
class AuxiliaryStreams1(nn.Module):
    def __init__(self, channels: int, seq_len: int, pred_len: int):
        ...
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.conv = DepthwiseSeparableConv1d(channels, kernel_size=5, dilation=2)
    ...
    def forward(self, x):  # x: [B,S,F]
        trend = self.moving_avg(x, kernel_size=3)
        seasonal = x - trend
...
def main(
    ...
    if auto_grid is None:
        auto_grid = [1.0,0.5,0.2,0.05,0.01]
    if nhits_grid is None:
        nhits_grid = [0.0,0.01,0.05, 0.1,0.2,1.0]
...
if __name__ == "__main__":
    ...
    #
    # Example:
    # python script.py I_5.csv results_DENet models_DENet None 20 "[0,0.05,0.1,0.2,0.5]" "[0,0.001,0.01,0.05,0.1]"
