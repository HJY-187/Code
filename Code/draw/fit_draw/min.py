...
# Set global plotting font to Times New Roman
...
class DENet(nn.Module):
    """
    Model class with ablation capabilities
    """
    ...
    def forward(self, x):
        ...
        # 3. N-HiTS Path (Modeling Residuals of Close Price)
        if self.use_nhits:
...
def train_and_evaluate(model_name, model, train_loader, test_loader, scaler, epochs=30, verbose=True, plot_title=None):
    if verbose:
        print(f"\nTraining {model_name}...")
    ...
    # --- Final Evaluation and Denormalization ---
    ...
    # Denormalize using RobustScaler
    ...
    if verbose:
        plot_len = 400
        # Take Step 1 of prediction sequence for comparison
        vis_true = trues[:, 0][-plot_len:] if len(trues) > plot_len else trues[:, 0]
        vis_pred = preds[:, 0][-plot_len:] if len(preds) > plot_len else preds[:, 0]

        plt.figure(figsize=(12, 6))
        plt.plot(vis_true, label='Ground Truth', color='tab:blue', linewidth=1.5)
        plt.plot(vis_pred, label='Prediction', color='tab:orange', linewidth=1.5)

        # Use passed plot_title, or model_name if not provided
        final_title = plot_title if plot_title else model_name
        plt.title(final_title, fontsize=16, fontweight='bold')

        plt.xlabel("Time Step (Last 400 points)", fontsize=12)
        ...
        plt.close()
        print(f"[{model_name}] Plot saved to {plot_path}")
    # ====================================================
    ...
    metrics = {
        ...
        "Step_Metrics": [],
        "preds": preds,  # Save predictions for subsequent file output
        "trues": trues
    }
...
def main():
    ...
    # 2. Configure full model (using optimized default parameters)
    model_config = {
        "dropout": 0.3,
        "nhits_hidden_ratio": 4,
        "nhits_init": 0.001,
        "auto_init": 0.1,
        "use_nhits": True,  # Enable N-HiTS path
        "use_auto": True    # Enable Autoformer path
    }
    ...
    # Pass specific title 'prediction vs true+DENet'
    metrics, trained_model = train_and_evaluate(
        ...
        epochs=50,
        verbose=True,
        plot_title="prediction vs true+DENet(J)"
    )
    ...
    print("-" * 80)
    print(f"{'DENet Full Model':<30} | {metrics['Overall_MAE']:.6f} | {metrics['Overall_RMSE']:.6f} | {metrics['Overall_R2']:.6f}")

    print("\n" + "=" * 80)
    print("Detailed Step Analysis (Step 1, 6, 12 RMSE)")
    print("-" * 80)
    s1 = metrics['Step_Metrics'][0]['RMSE']
    s6 = metrics['Step_Metrics'][5]['RMSE']
    s12 = metrics['Step_Metrics'][11]['RMSE']
    print(f"{'DENet Full Model':<30} | {s1:.6f} | {s6:.6f} | {s12:.6f}")

    # 5. Generate final prediction file
    print("\n" + "=" * 60)
    ...
    save_path = os.path.join("../..", "draw/min", "pred_minute_advanced_opt_J.csv")
