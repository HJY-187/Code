...
def preprocess_data(df: pd.DataFrame, look_back: int):
    ...
    return train_loader, test_loader, scaler, len(feature_cols)
...
class AblationModel(nn.Module):
    ...
    def __init__(self, seq_len: int, in_features: int,
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(decoder_dropout),  # Use dynamic dropout
...
class DirectionalLoss(nn.Module):
    ...
    def forward(self, pred, true, last_close):
        ...
        # Ensure shapes align
        # true/pred: [B, 1], last_close: [B] -> [B, 1]
...
# ==============================================================
def run_experiment(exp_name, use_nhits, use_auto, loaders, scaler, num_features, **model_kwargs):

    print(f"\n[{exp_name}] Starting... (N-HiTS={use_nhits}, Auto={use_auto})")
    ...
    model = AblationModel(
        ...
        use_nhits=use_nhits,
        use_auto=use_auto,
        # Pass everything in model_kwargs, but need to extract custom args if AblationModel doesn't accept them in **kwargs directly
        # Actually AblationModel init needs update to accept 'decoder_dropout' if we want to search it.
        # Let's adding decoder_dropout to AblationModel init in next chunk.
        **model_kwargs
    ).to(DEVICE)
    # Actually, let's rewrite run_experiment to accept **kwargs
    pass  # Placeholder to signify I will replace the whole function in next chunk

    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=1e-4)
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=8)
    criterion = DirectionalLoss(alpha=2.0)  # Use Directional Loss
...
if __name__ == "__main__":
    ...
    #  preprocess_data
    train_loader, test_loader, scaler, num_features = preprocess_data(df, LOOK_BACK)
    ...
    def random_search_daily(loaders, scaler, num_features, num_trials=6):
        ...
        param_grid = {
            ...
            # Fine-tune weights to favor Full Model synergy
            "nhits_init": [0.01, 0.05, 0.08],
            "auto_init": [0.15, 0.2, 0.25],
            # DLinear decoder regularization
        ...
        for i in range(num_trials):
            ...
            # Note: run_experiment does full training/eval.
            # We can use it directly but suppress some output or just parse its return.
            res = run_experiment(f"Trial_{i}", True, True, loaders, scaler, num_features, **config)
    ...
    #  1: base
    # Just pass best_params (ignored by model but safe) or separate
    res_base = run_experiment("Base", False, False, loaders, scaler, num_features, **best_params)
    results.append(res_base)

    #  2: base + AS1
    res_auto = run_experiment("Base + AS1", False, True, loaders, scaler, num_features, **best_params)
    results.append(res_auto)

    #  3: base + AS2 (Optimized)
    res_nhits = run_experiment("Base + AS2", True, False, loaders, scaler, num_features, **best_params)
    results.append(res_nhits)

    #  4:full (Optimized)
    res_full = run_experiment("DENet", True, True, loaders, scaler, num_features, **best_params)
...
Base,12.4861,16.7595,0.9568
Base + AS1,12.4631 16.6234 0.9572
Base + AS2,12.4446 16.7702 0.9571
DENet 12.4277 16.6054 0.9575
"""