...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # Target: close value (index 3) of the next trading day
        y = self.data[idx + self.seq_len, 3]

        # Closing price at current time (for trend accuracy calculation)
...
def preprocess_data(df: pd.DataFrame, look_back: int):
    ...
    # 1. Log transformation (price-related fields)
...
# ============================== Model Definition (Main Path Only) ==============================
...
class PMain(nn.Module):
    """
    Simplified DLinear Main Path:
    • Feature weighting
    • Moving average decomposition (trend / residual)
    • Separate linear projection
    • Fusion + small MLP refinement
    """
    ...
    def forward(self, x):           # x: [B, L, in_features]
        ...
        # Classic DLinear decomposition
        trend = self.moving_avg(weighted_x)
        residual = weighted_x - trend

        # Project to prediction length separately
        ...
        # Only take prediction of close channel (index 3)
        base_pred = (trend_pred + residual_pred)[:, :, 3]       # [B, P]

        # Optional refinement layer (usually better performance)
